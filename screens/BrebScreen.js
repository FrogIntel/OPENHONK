...
const BrebScreen = ({ navigation }) => {
  ...
  // Get random websites from all breb categories for carousel
  const getRandomWebsites = () => {
    const allWebsites = [];
    Object.keys(brebData).forEach(subCategory => {
      const categoryData = brebData[subCategory];
...
👉 https://github.com/FrogIntel`,
  ...
  const openCategory = (categoryKey, categoryTitle) => {
    navigation.navigate('Category', { categoryKey, categoryTitle, data: brebData });
  };
  ...
  return (
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: theme.primaryColor }]}>BREB</Text>
